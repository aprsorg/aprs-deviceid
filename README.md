...
Beginning from 2022-02-19, this is the primary database for APRS device
...
strings.  For easy manual editing and validation, the primary source file is
in YAML format.  A generator tool, provided in this repository, checks the
YAML file for correctness and converts the database to XML and JSON.  The
generated versions are also made available for download.
...
change in the primary source database, at the following URLs.  APRS
applications can automatically fetch fresh versions, for example, once per
week.  Just remember to pick a random day and time - don't schedule all
instances to download precisely on the same second.
...
metadata segment (generation timestamp!).

The [TOCALL Search](https://aprsorg.github.io/aprs-deviceid-web/) tool loads
the fresh JSON database and allows quick searches.  Use it to check for
collisions before requesting an allocation.
...
The source file is tocalls.yaml, and all the other files are generated from
